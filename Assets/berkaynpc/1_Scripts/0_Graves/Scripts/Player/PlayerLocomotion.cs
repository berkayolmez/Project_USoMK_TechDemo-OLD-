        ...
        private void HandleClimbing()
        {
            if (playerManager.isClimbing)
            {
               if (inputHandler.climbFlag)
               {
                ...
                transform.rotation = Quaternion.Lerp(transform.rotation, Quaternion.LookRotation(-climbHit.normal), 0.4f);  //duvara dogur bak  //eger kamera yamuluyorsa bundan dolayý olabilir
                ...
                if (moveAmount > 0)
                {
                    ...
                    cController.Move(transform.forward * climbSpeed / 2 * Time.deltaTime); //duvara dogru itiyor***    
                }
            ...
            else if (!playerManager.isClimbing)
            {
                ...
                {
                    if (playerInteractor.bodyInteractor(Bodyparts.headCenter, climbableWallMask, 0.5f) || playerInteractor.bodyInteractor(Bodyparts.hipsCenter, climbableWallMask, 0.5f))
                    {
                        if (!playerManager.inAnim)
                        {
