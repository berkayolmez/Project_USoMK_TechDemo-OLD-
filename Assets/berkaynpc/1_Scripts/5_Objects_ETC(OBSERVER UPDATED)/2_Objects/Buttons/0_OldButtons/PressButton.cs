...
namespace project_usomk
{
    ...
    {                                                              //OTHER SCRIPTS AND INTERFACES      
                ...
                switch (buttonType)
                {
                    case ButtonType.PressNoTimer:
                        ...
                        if (canPress)
                        {
                            ...
                            InteractedMe(pressButtonStatus);
                        }
                        break;
        ...
        private void InteractedMe(bool getBool)                   //IF PRESS to THIS BUTTON SET ALL connectedGameObj's STATUS  //bu rpc ye gidebilir mi ???????? 
        {
            StartCoroutine("WaitForTimer");
            myFunctions.SetMyConnectedObjects(connectedGameObjs, getBool);             
        }

        public void IsStillPressing(bool buttonStatus)               // CHECK PLAYER STILL HOLDING F KEY
        {
            switch (buttonType)
            {
                case ButtonType.PressAndHold:
        ...
        public void PressedButton(bool isButtonOn) //controller objects send this
        {
            ...
            if (!controllerStatus)
            {
                Debug.Log("pressbutton icindeyiz " + controllerStatus);
